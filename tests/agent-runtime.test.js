...
test('AgentRuntime prepares exactly one internal STAGE instruction', () => {
  ...
  fs.mkdirSync(path.join(root, 'skills', 'knowledge-orchestrator', 'stages'), { recursive: true });
  ...
  fs.writeFileSync(path.join(root, 'skills', 'knowledge-orchestrator', 'stages.json'), JSON.stringify({
    schema_version: '0.6.1', stages: { router: { instruction: 'skills/knowledge-orchestrator/stages/router.md',
      include_raw: true, artifacts: [] } }
  }));
  fs.writeFileSync(path.join(root, 'skills', 'knowledge-orchestrator', 'stages', 'router.md'), '# Router internal instruction');
  ...
  assert.equal(prepared.request.stage_instruction.path, 'skills/knowledge-orchestrator/stages/router.md');
