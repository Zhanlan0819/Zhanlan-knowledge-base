...
# 知识整理总控 v0.7.1｜Full-Knowledge Contract
...
## 13. 完整知识契约：正式知识不能只是摘要

正式知识的目标是“打开这一条就能学会和使用”，不是“知道还有哪些来源可以去翻”。

必须遵守：

- 隐藏所有 Sources 后，正文仍应独立可理解、可执行；
- 引用只负责溯源，不能代替正文承载知识；
- 每个 Theme 的全部 Claim 必须在 Distiller 中明确去向，禁止静默丢失；
- N点法、N步法、SOP、表格、清单、模板、框架、模型、公式、话术结构等高信息密度内容必须单独识别；
- 只要这些结构对当前 Theme 必要，就必须在正文完整展开，不能只写“参考 XX / 详见 XX / 可用 XX”；
- 允许去重、重组和压缩重复案例，但步骤、分类、判断标准、关键字段和代表案例不能无理由消失；
- Proposal 进入待确认前必须通过知识完整性审计。审计时默认 Sources 全部不可见；
- 任何必要知识缺失、必要结构未展开、引用替代正文的情况，都必须返工，不能进入待确认。

Review Compression 只用于减少人工审核，不用于压缩知识本身。
