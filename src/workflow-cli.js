...
try {
  ...
  const service = new WorkflowService(store, { reviewerSecret: process.env.KB_REVIEW_SECRET ?? null });
  if (action === 'start' || action === 'demo') {
    ...
    if (action === 'start') print({ run_id: runId, state_path: service.statePath(runId), next_stage: 'router',
      suggestion_run_id: started.state.suggestion_run_id });
    else {
      ...
      print({ run_id: runId, status: r.status, checkpoint: r.checkpoint, reason: r.reason,
    ...
    if (action === 'status' || action === 'resume') {
      const r = service.resume(runId);
      print({ run_id: runId, status: r.status, next_stage: r.next_stage,
      ...
      if (!process.stdin.isTTY || !process.env.KB_REVIEW_SECRET)
        throw new Error('此命令只允许真人审阅终端运行，并需由审阅服务提供 KB_REVIEW_SECRET');
      if (action === 'decision') {
        ...
        print({ run_id: runId, decision_id: result.decision.id, state_status: result.state.status });
    ...
    } else throw new Error('可用命令: start, demo, status, resume, submit, retry, fail, record-output, decision, apply, publish, rollback, trace');
  }
